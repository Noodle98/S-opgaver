...
class Customer(Base):
    ...
    def __repr__(self):
        return f'Person: id: {self.id}     name: {self.name}      address: {self.address}      age: {self.age}'
...
def create_test_data():
    with Session(engine) as session:
        new_items = []
        new_items.append(Customer(name="jonas", address="byvej 12", age=24))
        new_items.append(Customer(name="frank", address="nærheden 12", age=44))
        new_items.append(Customer(name="ulla", address="østerparkvej 18", age=54))
